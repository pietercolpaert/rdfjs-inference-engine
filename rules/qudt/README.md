...
For that reason this folder includes `qudt-cdt-normalization.runtime.n3`, a checked-in generated runtime projection containing the QUDT unit statements used by this profile. The build filters out similarly named metadata on quantity-kind and other resources because normalization only consults `unit:` resources. `InferenceEngine` activates this precompiled runtime automatically when the QUDT profile is selected by itself, or when a mixed-profile load-time vocabulary contains QUDT/CDT terms. It stays dormant for unrelated mixed OWL/SKOS loads.

When SHACL IN and SHACL OUT provide unit constraints, the engine specializes this projection before attaching it. It reads scalar `sh:unit` annotations, CDT alternatives from `qcr:UcumUnitIn`, and `sh:hasValue`/`sh:in` constraints on `qudt:unit`. Exact input constraints retain only the named source and target units. If SHACL IN intentionally leaves the source unit open, all units sharing a QUDT dimension vector with the requested output unit are retained. The specialization also keeps only the datatype profiles and forward rule families required by the SHACL input representation and output conversion category; incomplete or ambiguous shapes retain the conservative general kernel.

Local unit identifiers are resolved through load-time OWL `owl:sameAs`, SKOS `skos:exactMatch`, or `qcr:alignedQudtUnit` facts before specialization. This also applies to local tokens inside CDT literals: a local unit declares its lexical token with `qudt:symbol`, while OWL or SKOS aligns that unit IRI with the QUDT unit carrying conversion metadata.
