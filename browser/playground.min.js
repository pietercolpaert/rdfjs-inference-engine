"use strict";(()=>{var D=(e,t,r)=>()=>{if(r)throw r[0];try{return e&&(t=e(e=0)),t}catch(a){throw r=[a],a}};var Y=(e,t)=>()=>{try{return t||e((t={exports:{}}).exports,t),t.exports}catch(r){throw t=0,r}};var O,F,N=D(()=>{O=["owl2rl-eyeling.n3","skos-entailment.n3"],F=`# Source: rules/owl2rl-eyeling.n3
...
`},{id:"inconsistency-diagnostics",label:"Inconsistency diagnostics (RDF Messages)",backgroundFile:"examples/inconsistency-diagnostics/ontology.n3",dataFile:"examples/inconsistency-diagnostics/input.messages.trig",background:`# Background ontology for the inconsistency diagnostics playground example.

@prefix : <https://example.org/test#> .
@prefix owl: <http://www.w3.org/2002/07/owl#> .

:A owl:disjointWith :B .
`,data:`# RDF Messages input for the inconsistency diagnostics playground example.
# Stateful materialization is enabled for this bundled example so the two
# facts below are considered together across messages.
...
`}]});var ye=Y(()=>{N();q();var d={backgroundMode:"text",dataMode:"text",backgroundText:y().background,dataText:y().data},s={backgroundText:P("backgroundText",d.backgroundText),dataText:P("dataText",d.dataText),outputText:P("outputText","")},n={exampleSelect:E("exampleSelect"),backgroundMode:E("backgroundMode"),dataMode:E("dataMode"),statefulMaterialization:C("statefulMaterialization"),backgroundUrl:C("backgroundUrl"),dataUrl:C("dataUrl"),backgroundUrlPanel:p("backgroundUrlPanel"),backgroundTextPanel:p("backgroundTextPanel"),dataUrlPanel:p("dataUrlPanel"),dataTextPanel:p("dataTextPanel"),runButton:v("runButton"),stopButton:v("stopButton"),resetButton:v("resetButton"),shareButton:v("shareButton"),status:p("status"),runtimeStats:p("runtimeStats"),rulesSummary:we("rulesSummary")},c=!1,j=0,w=null,h="",u=0;n.rulesSummary&&(n.rulesSummary.textContent=`Bundled inference profiles: ${O.join(", ")||"none"}`);le();ce();T();H();g();L(!1);m("Ready. Choose an example, URL, or text input, then run OWL 2 RL + SKOS Core inference.");function P(e,t){let r=document.getElementById(e);if(!r)throw new Error(`Missing textarea #${e}`);return r.value=t,CodeMirror.fromTextArea(r,{mode:"text/turtle",lineNumbers:!0,lineWrapping:!0,tabSize:2,viewportMargin:1/0})}function H(){n.runButton.addEventListener("click",()=>{$()}),n.stopButton.addEventListener("click",Z),n.resetButton.addEventListener("click",oe),n.exampleSelect.addEventListener("change",()=>ie(n.exampleSelect.value)),n.shareButton.addEventListener("click",()=>{R(),navigator.clipboard?.writeText(window.location.href),m("Shareable URL copied when clipboard access is available.")});for(let e of[n.backgroundMode,n.dataMode])e.addEventListener("change",()=>{T(),g()});for(let e of Object.values(s))e.on("change",g);for(let e of[n.backgroundUrl,n.dataUrl])e.addEventListener("input",g);n.statefulMaterialization.addEventListener("change",g)}async function $(){if(w)return;let e={controller:new AbortController,startedAt:performance.now()};w=e;try{L(!0),n.runtimeStats.textContent="",n.runtimeStats.hidden=!0,_(),ee(e),s.outputText.setValue(""),z(e,"Preparing inference\u2026");let t=await J("background",e.controller.signal);se(e.controller.signal);let r=i("data"),a=r==="text"?s.dataText.getValue():void 0,o=r==="url"?n.dataUrl.value.trim():void 0;if(r==="url"&&!o)throw new Error("Enter a data URL or switch to text input.");await K(e,{apiScriptUrl:new URL("browser/rdfjs-inference-engine.min.js",window.location.href).href,bundledRules:F,bundledRuleCount:O.length,backgroundSource:t,dataMode:r,statefulMaterialization:n.statefulMaterialization.checked,statefulStoreName:n.statefulMaterialization.checked?X(t,r==="url"?o??"":a??""):void 0,dataSource:a,dataUrl:o})}catch(t){if(ae(t))A(),e.statusMessage="Stopped. No more messages or quads will be processed.";else{let r=t instanceof Error?t.message:String(t);_(),e.statusMessage=`Error: ${r}`,s.outputText.setValue(r)}M(e)}finally{M(e),w===e&&(w=null),e.worker?.terminate(),L(!1)}}function K(e,t){return new Promise((r,a)=>{let o=G();e.worker=o;let f=()=>{o.terminate(),a(new DOMException("Inference was stopped.","AbortError"))};e.controller.signal.addEventListener("abort",f,{once:!0}),o.onmessage=S=>{let l=S.data;l.type==="status"?z(e,l.message):l.type==="runtime"?(e.runtimeMessage=l.message,k(e)):l.type==="append"?te(l.chunk):l.type==="result"?(e.statusMessage=l.status,l.output!==void 0?(_(),s.outputText.setValue(l.output)):A(),M(e),r()):l.type==="error"&&a(new Error(l.message))},o.onerror=S=>{a(new Error(S.message))},o.postMessage(t)})}function X(e,t){let r=[window.location.origin,window.location.pathname,e,t].join("\0");return`rdfjs-inference-engine:playground:${Q(r)}`}function Q(e){let t=3735928559,r=1103547991;for(let a=0;a<e.length;a+=1){let o=e.charCodeAt(a);t=Math.imul(t^o,2654435761),r=Math.imul(r^o,1597334677)}return t=Math.imul(t^t>>>16,2246822507)^Math.imul(r^r>>>13,3266489909),r=Math.imul(r^r>>>16,2246822507)^Math.imul(t^t>>>13,3266489909),`${(r>>>0).toString(36)}${(t>>>0).toString(36)}`}function G(){let e=`
...
`,t=new Blob([e],{type:"text/javascript"});return new Worker(URL.createObjectURL(t))}async function J(e,t){if(i(e)==="text")return e==="background"?s.backgroundText.getValue():s.dataText.getValue();let o=(e==="background"?n.backgroundUrl:n.dataUrl).value.trim();if(!o)throw new Error(`Enter a ${e==="background"?"background RDF":"data"} URL or switch to text input.`);m(`Fetching ${e==="background"?"background RDF":"input data"} before processing\u2026`);let f=await fetch(o,{signal:t});if(!f.ok)throw new Error(`Could not fetch ${o}: ${f.status} ${f.statusText}`);return f.text()}function Z(){w&&(m("Stopping after the current worker step\u2026"),n.stopButton.disabled=!0,w.controller.abort())}function L(e){n.runButton.disabled=e,n.stopButton.hidden=!e,n.stopButton.disabled=!e}function z(e,t){e.statusMessage=t,V(e)}function ee(e){k(e),e.elapsedTimer=window.setInterval(()=>k(e),250)}function M(e){e.finishedAt===void 0&&(e.finishedAt=performance.now()),e.elapsedTimer!==void 0&&(window.clearInterval(e.elapsedTimer),e.elapsedTimer=void 0),k(e)}function k(e){n.runtimeStats.textContent="",n.runtimeStats.hidden=!0,V(e)}function V(e){if(!e.statusMessage)return;let t=e.finishedAt===void 0?"Elapsed":"Total elapsed",r=[W(e.statusMessage),`${t} ${re(ne(e))}`];e.runtimeMessage&&r.push(W(e.runtimeMessage)),m(r.filter(Boolean).join(" \xB7 "))}function W(e){return e.trim().replace(/[.。]\s*$/,"")}function te(e){h+=e,u||(u=window.setTimeout(A,100))}function A(){if(u&&(window.clearTimeout(u),u=0),!h)return;let e=h;if(h="",s.outputText.replaceRange&&s.outputText.lastLine&&s.outputText.getLine){let t=s.outputText.lastLine();s.outputText.replaceRange(e,{line:t,ch:s.outputText.getLine(t).length})}else s.outputText.setValue(s.outputText.getValue()+e)}function _(){u&&(window.clearTimeout(u),u=0),h=""}function ne(e){return(e.finishedAt??performance.now())-e.startedAt}function re(e){let t=Math.max(0,e/1e3);if(t<10)return`${t.toFixed(1)} s`;if(t<60)return`${t.toFixed(0)} s`;let r=Math.floor(t/60),a=Math.floor(t%60).toString().padStart(2,"0");return`${r} min ${a} s`}function se(e){if(e.aborted)throw new DOMException("Inference was stopped.","AbortError")}function ae(e){return e instanceof DOMException&&e.name==="AbortError"}function T(){b(n.backgroundUrlPanel,i("background")==="url"),b(n.backgroundTextPanel,i("background")==="text"),b(n.dataUrlPanel,i("data")==="url"),b(n.dataTextPanel,i("data")==="text"),window.setTimeout(()=>{s.backgroundText.refresh(),s.dataText.refresh(),s.outputText.refresh()},0)}function b(e,t){e.hidden=!t}function i(e){return(e==="background"?n.backgroundMode.value:n.dataMode.value)==="url"?"url":"text"}function oe(){let e=y();c=!0,n.exampleSelect.value=e.id,n.backgroundMode.value=d.backgroundMode,n.dataMode.value=d.dataMode,n.statefulMaterialization.checked=!1,n.backgroundUrl.value="",n.dataUrl.value="",s.backgroundText.setValue(e.background),s.dataText.setValue(e.data),s.outputText.setValue(""),c=!1,T(),R(),m(`Reset to ${e.label}.`)}function le(){n.exampleSelect.textContent="";for(let e of x){let t=document.createElement("option");t.value=e.id,t.textContent=`${e.label} \u2014 ${e.backgroundFile} + ${e.dataFile}`,n.exampleSelect.appendChild(t)}n.exampleSelect.value=y().id}function ie(e){let t=I(e)??y();c=!0,B(t),c=!1,T(),R(),m(`Loaded ${t.label} from ${t.backgroundFile} and ${t.dataFile}.`)}function B(e){n.exampleSelect.value=e.id,n.backgroundMode.value="text",n.dataMode.value="text",n.statefulMaterialization.checked=U(e.id),n.backgroundUrl.value="",n.dataUrl.value="",s.backgroundText.setValue(e.background),s.dataText.setValue(e.data),s.outputText.setValue("")}function U(e){return e==="stateful-materialization"||e==="inconsistency-diagnostics"}function y(){return I("owl-skos-catalog")??x[0]}function I(e){return x.find(t=>t.id===e)}function de(){let e=I(n.exampleSelect.value);if(e&&pe(e))return e.id===y().id?{}:{example:e.id};let t={backgroundMode:i("background")===d.backgroundMode?void 0:i("background"),dataMode:i("data")===d.dataMode?void 0:i("data"),statefulMaterialization:n.statefulMaterialization.checked||void 0,backgroundUrl:n.backgroundUrl.value.trim()||void 0,dataUrl:n.dataUrl.value.trim()||void 0},r=s.backgroundText.getValue(),a=s.dataText.getValue();return r!==d.backgroundText&&(t.backgroundText=r),a!==d.dataText&&(t.dataText=a),t}function pe(e){return i("background")==="text"&&i("data")==="text"&&n.statefulMaterialization.checked===U(e.id)&&n.backgroundUrl.value.trim()===""&&n.dataUrl.value.trim()===""&&s.backgroundText.getValue()===e.background&&s.dataText.getValue()===e.data}function ce(){let e=me(window.location.hash);if(e){let r=I(e);if(r){c=!0,B(r),c=!1;return}}let t=fe(window.location.hash);t&&(c=!0,n.backgroundMode.value=t.backgroundMode??d.backgroundMode,n.dataMode.value=t.dataMode??d.dataMode,n.statefulMaterialization.checked=t.statefulMaterialization??!1,n.backgroundUrl.value=t.backgroundUrl??"",n.dataUrl.value=t.dataUrl??"",t.backgroundText!==void 0&&s.backgroundText.setValue(t.backgroundText),t.dataText!==void 0&&s.dataText.setValue(t.dataText),c=!1)}function g(){c||(window.clearTimeout(j),j=window.setTimeout(R,400))}function R(){let e=ue(de()),t=`${window.location.pathname}${window.location.search}${e?`#${e}`:""}`;window.history.replaceState(null,"",t)}function ue(e){if(e.example&&Object.keys(e).length===1)return`example=${encodeURIComponent(e.example)}`;let t=JSON.stringify(e);return t==="{}"?"":`state=${btoa(unescape(encodeURIComponent(t))).replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/g,"")}`}function me(e){if(!e.startsWith("#example="))return"";try{return decodeURIComponent(e.slice(9))}catch{return""}}function fe(e){let t=e.startsWith("#state=")?e.slice(7):"";if(!t)return null;try{let r=t.replace(/-/g,"+").replace(/_/g,"/").padEnd(Math.ceil(t.length/4)*4,"=");return JSON.parse(decodeURIComponent(escape(atob(r))))}catch{return null}}function m(e){n.status.textContent=e}function C(e){return p(e)}function E(e){return p(e)}function v(e){return p(e)}function p(e){let t=document.getElementById(e);if(!t)throw new Error(`Missing element #${e}`);return t}function we(e){return document.getElementById(e)}});ye();})();
