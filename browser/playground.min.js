"use strict";(()=>{var A=(e,t,r)=>()=>{if(r)throw r[0];try{return e&&(t=e(e=0)),t}catch(a){throw r=[a],a}};var B=(e,t)=>()=>{try{return t||e((t={exports:{}}).exports,t),t.exports}catch(r){throw t=0,r}};var S,D,F=A(()=>{S=["owl2rl-eyeling.n3","skos-entailment.n3"],D=`# Source: rules/owl2rl-eyeling.n3
...
=> { ?left skos:exactMatch ?right . } .`});var b,N=A(()=>{b=[{id:"owl-skos-catalog",label:"Catalog topics (OWL 2 RL + SKOS Core)",backgroundFile:"examples/owl-skos-catalog/ontology.n3",dataFile:"examples/owl-skos-catalog/input.trig",background:`# Combined OWL 2 RL + SKOS Core example background.
#
...
`}]});var me=B(()=>{F();N();var i={backgroundMode:"text",dataMode:"text",backgroundText:h().background,dataText:h().data},s={backgroundText:O("backgroundText",i.backgroundText),dataText:O("dataText",i.dataText),outputText:O("outputText","")},n={exampleSelect:C("exampleSelect"),backgroundMode:C("backgroundMode"),dataMode:C("dataMode"),statefulMaterialization:P("statefulMaterialization"),backgroundUrl:P("backgroundUrl"),dataUrl:P("dataUrl"),backgroundUrlPanel:p("backgroundUrlPanel"),backgroundTextPanel:p("backgroundTextPanel"),dataUrlPanel:p("dataUrlPanel"),dataTextPanel:p("dataTextPanel"),runButton:v("runButton"),stopButton:v("stopButton"),resetButton:v("resetButton"),shareButton:v("shareButton"),status:p("status"),runtimeStats:p("runtimeStats"),rulesSummary:ue("rulesSummary")},u=!1,q=0,w=null,g="",c=0;n.rulesSummary&&(n.rulesSummary.textContent=`Bundled inference profiles: ${S.join(", ")||"none"}`);ae();de();k();U();y();L(!1);m("Ready. Choose an example, URL, or text input, then run OWL 2 RL + SKOS Core inference.");function O(e,t){let r=document.getElementById(e);if(!r)throw new Error(`Missing textarea #${e}`);return r.value=t,CodeMirror.fromTextArea(r,{mode:"text/turtle",lineNumbers:!0,lineWrapping:!0,tabSize:2,viewportMargin:1/0})}function U(){n.runButton.addEventListener("click",()=>{Y()}),n.stopButton.addEventListener("click",G),n.resetButton.addEventListener("click",se),n.exampleSelect.addEventListener("change",()=>oe(n.exampleSelect.value)),n.shareButton.addEventListener("click",()=>{I(),navigator.clipboard?.writeText(window.location.href),m("Shareable URL copied when clipboard access is available.")});for(let e of[n.backgroundMode,n.dataMode])e.addEventListener("change",()=>{k(),y()});for(let e of Object.values(s))e.on("change",y);for(let e of[n.backgroundUrl,n.dataUrl])e.addEventListener("input",y);n.statefulMaterialization.addEventListener("change",y)}async function Y(){if(w)return;let e={controller:new AbortController,startedAt:performance.now()};w=e;try{L(!0),n.runtimeStats.textContent="",n.runtimeStats.hidden=!0,M(),J(e),s.outputText.setValue(""),W(e,"Preparing inference\u2026");let t=await Q("background",e.controller.signal);ne(e.controller.signal);let r=d("data"),a=r==="text"?s.dataText.getValue():void 0,o=r==="url"?n.dataUrl.value.trim():void 0;if(r==="url"&&!o)throw new Error("Enter a data URL or switch to text input.");await H(e,{apiScriptUrl:new URL("browser/rdfjs-inference-engine.min.js",window.location.href).href,bundledRules:D,bundledRuleCount:S.length,backgroundSource:t,dataMode:r,statefulMaterialization:n.statefulMaterialization.checked,statefulStoreName:n.statefulMaterialization.checked?$(t,r==="url"?o??"":a??""):void 0,dataSource:a,dataUrl:o})}catch(t){if(re(t))_(),e.statusMessage="Stopped. No more messages or quads will be processed.";else{let r=t instanceof Error?t.message:String(t);M(),e.statusMessage=`Error: ${r}`,s.outputText.setValue(r)}E(e)}finally{E(e),w===e&&(w=null),e.worker?.terminate(),L(!1)}}function H(e,t){return new Promise((r,a)=>{let o=X();e.worker=o;let f=()=>{o.terminate(),a(new DOMException("Inference was stopped.","AbortError"))};e.controller.signal.addEventListener("abort",f,{once:!0}),o.onmessage=R=>{let l=R.data;l.type==="status"?W(e,l.message):l.type==="runtime"?(e.runtimeMessage=l.message,T(e)):l.type==="append"?Z(l.chunk):l.type==="result"?(e.statusMessage=l.status,l.output!==void 0?(M(),s.outputText.setValue(l.output)):_(),E(e),r()):l.type==="error"&&a(new Error(l.message))},o.onerror=R=>{a(new Error(R.message))},o.postMessage(t)})}function $(e,t){let r=[window.location.origin,window.location.pathname,e,t].join("\0");return`rdfjs-inference-engine:playground:${K(r)}`}function K(e){let t=3735928559,r=1103547991;for(let a=0;a<e.length;a+=1){let o=e.charCodeAt(a);t=Math.imul(t^o,2654435761),r=Math.imul(r^o,1597334677)}return t=Math.imul(t^t>>>16,2246822507)^Math.imul(r^r>>>13,3266489909),r=Math.imul(r^r>>>16,2246822507)^Math.imul(t^t>>>13,3266489909),`${(r>>>0).toString(36)}${(t>>>0).toString(36)}`}function X(){let e=`
...
`,t=new Blob([e],{type:"text/javascript"});return new Worker(URL.createObjectURL(t))}async function Q(e,t){if(d(e)==="text")return e==="background"?s.backgroundText.getValue():s.dataText.getValue();let o=(e==="background"?n.backgroundUrl:n.dataUrl).value.trim();if(!o)throw new Error(`Enter a ${e==="background"?"background RDF":"data"} URL or switch to text input.`);m(`Fetching ${e==="background"?"background RDF":"input data"} before processing\u2026`);let f=await fetch(o,{signal:t});if(!f.ok)throw new Error(`Could not fetch ${o}: ${f.status} ${f.statusText}`);return f.text()}function G(){w&&(m("Stopping after the current worker step\u2026"),n.stopButton.disabled=!0,w.controller.abort())}function L(e){n.runButton.disabled=e,n.stopButton.hidden=!e,n.stopButton.disabled=!e}function W(e,t){e.statusMessage=t,z(e)}function J(e){T(e),e.elapsedTimer=window.setInterval(()=>T(e),250)}function E(e){e.finishedAt===void 0&&(e.finishedAt=performance.now()),e.elapsedTimer!==void 0&&(window.clearInterval(e.elapsedTimer),e.elapsedTimer=void 0),T(e)}function T(e){n.runtimeStats.textContent="",n.runtimeStats.hidden=!0,z(e)}function z(e){if(!e.statusMessage)return;let t=e.finishedAt===void 0?"Elapsed":"Total elapsed",r=[j(e.statusMessage),`${t} ${te(ee(e))}`];e.runtimeMessage&&r.push(j(e.runtimeMessage)),m(r.filter(Boolean).join(" \xB7 "))}function j(e){return e.trim().replace(/[.。]\s*$/,"")}function Z(e){g+=e,c||(c=window.setTimeout(_,100))}function _(){if(c&&(window.clearTimeout(c),c=0),!g)return;let e=g;if(g="",s.outputText.replaceRange&&s.outputText.lastLine&&s.outputText.getLine){let t=s.outputText.lastLine();s.outputText.replaceRange(e,{line:t,ch:s.outputText.getLine(t).length})}else s.outputText.setValue(s.outputText.getValue()+e)}function M(){c&&(window.clearTimeout(c),c=0),g=""}function ee(e){return(e.finishedAt??performance.now())-e.startedAt}function te(e){let t=Math.max(0,e/1e3);if(t<10)return`${t.toFixed(1)} s`;if(t<60)return`${t.toFixed(0)} s`;let r=Math.floor(t/60),a=Math.floor(t%60).toString().padStart(2,"0");return`${r} min ${a} s`}function ne(e){if(e.aborted)throw new DOMException("Inference was stopped.","AbortError")}function re(e){return e instanceof DOMException&&e.name==="AbortError"}function k(){x(n.backgroundUrlPanel,d("background")==="url"),x(n.backgroundTextPanel,d("background")==="text"),x(n.dataUrlPanel,d("data")==="url"),x(n.dataTextPanel,d("data")==="text"),window.setTimeout(()=>{s.backgroundText.refresh(),s.dataText.refresh(),s.outputText.refresh()},0)}function x(e,t){e.hidden=!t}function d(e){return(e==="background"?n.backgroundMode.value:n.dataMode.value)==="url"?"url":"text"}function se(){let e=h();u=!0,n.exampleSelect.value=e.id,n.backgroundMode.value=i.backgroundMode,n.dataMode.value=i.dataMode,n.statefulMaterialization.checked=!1,n.backgroundUrl.value="",n.dataUrl.value="",s.backgroundText.setValue(e.background),s.dataText.setValue(e.data),s.outputText.setValue(""),u=!1,k(),I(),m(`Reset to ${e.label}.`)}function ae(){n.exampleSelect.textContent="";for(let e of b){let t=document.createElement("option");t.value=e.id,t.textContent=`${e.label} \u2014 ${e.backgroundFile} + ${e.dataFile}`,n.exampleSelect.appendChild(t)}n.exampleSelect.value=h().id}function oe(e){let t=V(e)??h();u=!0,n.backgroundMode.value="text",n.dataMode.value="text",n.statefulMaterialization.checked=le(t.id),n.backgroundUrl.value="",n.dataUrl.value="",s.backgroundText.setValue(t.background),s.dataText.setValue(t.data),s.outputText.setValue(""),u=!1,k(),I(),m(`Loaded ${t.label} from ${t.backgroundFile} and ${t.dataFile}.`)}function le(e){return e==="stateful-materialization"||e==="inconsistency-diagnostics"}function h(){return V("owl-skos-catalog")??b[0]}function V(e){return b.find(t=>t.id===e)}function ie(){let e={backgroundMode:d("background")===i.backgroundMode?void 0:d("background"),dataMode:d("data")===i.dataMode?void 0:d("data"),statefulMaterialization:n.statefulMaterialization.checked||void 0,backgroundUrl:n.backgroundUrl.value.trim()||void 0,dataUrl:n.dataUrl.value.trim()||void 0},t=s.backgroundText.getValue(),r=s.dataText.getValue();return t!==i.backgroundText&&(e.backgroundText=t),r!==i.dataText&&(e.dataText=r),e}function de(){let e=ce(window.location.hash);e&&(u=!0,n.backgroundMode.value=e.backgroundMode??i.backgroundMode,n.dataMode.value=e.dataMode??i.dataMode,n.statefulMaterialization.checked=e.statefulMaterialization??!1,n.backgroundUrl.value=e.backgroundUrl??"",n.dataUrl.value=e.dataUrl??"",e.backgroundText!==void 0&&s.backgroundText.setValue(e.backgroundText),e.dataText!==void 0&&s.dataText.setValue(e.dataText),u=!1)}function y(){u||(window.clearTimeout(q),q=window.setTimeout(I,400))}function I(){let e=pe(ie()),t=`${window.location.pathname}${window.location.search}${e?`#state=${e}`:""}`;window.history.replaceState(null,"",t)}function pe(e){let t=JSON.stringify(e);return t==="{}"?"":btoa(unescape(encodeURIComponent(t))).replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/g,"")}function ce(e){let t=e.startsWith("#state=")?e.slice(7):"";if(!t)return null;try{let r=t.replace(/-/g,"+").replace(/_/g,"/").padEnd(Math.ceil(t.length/4)*4,"=");return JSON.parse(decodeURIComponent(escape(atob(r))))}catch{return null}}function m(e){n.status.textContent=e}function P(e){return p(e)}function C(e){return p(e)}function v(e){return p(e)}function p(e){let t=document.getElementById(e);if(!t)throw new Error(`Missing element #${e}`);return t}function ue(e){return document.getElementById(e)}});me();})();
