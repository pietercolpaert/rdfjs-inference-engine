"use strict";(()=>{var _=(e,t,s)=>()=>{if(s)throw s[0];try{return e&&(t=e(e=0)),t}catch(a){throw s=[a],a}};var H=(e,t)=>()=>{try{return t||e((t={exports:{}}).exports,t),t.exports}catch(s){throw t=0,s}};var R,D,N=_(()=>{R=["owl2rl-eyeling.n3","shacl-core-eyeling.n3","skos-entailment.n3"],D=`# Source: rules/owl2rl-eyeling.n3
...
# Source: rules/shacl-core-eyeling.n3
...
=> { ?left skos:exactMatch ?right . } .`});var x,j=_(()=>{x=[{id:"owl-skos-catalog",label:"Catalog topics (OWL 2 RL + SKOS Core)",backgroundFile:"examples/owl-skos-catalog/ontology.n3",dataFile:"examples/owl-skos-catalog/input.trig",background:`# Combined OWL 2 RL + SKOS Core example background.
#
...
`}]});var we=H(()=>{N();j();var p={backgroundMode:"text",dataMode:"text",backgroundText:w().background,dataText:w().data},r={backgroundText:P("backgroundText",p.backgroundText),dataText:P("dataText",p.dataText),outputText:P("outputText","")},n={exampleSelect:L("exampleSelect"),backgroundMode:L("backgroundMode"),dataMode:L("dataMode"),statefulMaterialization:O("statefulMaterialization"),backgroundUrl:O("backgroundUrl"),dataUrl:O("dataUrl"),backgroundUrlPanel:d("backgroundUrlPanel"),backgroundTextPanel:d("backgroundTextPanel"),dataUrlPanel:d("dataUrlPanel"),dataTextPanel:d("dataTextPanel"),runButton:v("runButton"),stopButton:v("stopButton"),resetButton:v("resetButton"),shareButton:v("shareButton"),status:d("status"),runtimeStats:d("runtimeStats"),rulesSummary:he("rulesSummary")},c=!1,q=0,h=null,y="",u=0;n.rulesSummary&&(n.rulesSummary.textContent=`Bundled inference profiles: ${R.join(", ")||"none"}`);le();ce();k();Y();g();E(!1);m("Ready. Choose an example, URL, or text input, then run OWL 2 RL + SKOS Core inference.");function P(e,t){let s=document.getElementById(e);if(!s)throw new Error(`Missing textarea #${e}`);return s.value=t,CodeMirror.fromTextArea(s,{mode:"text/turtle",lineNumbers:!0,lineWrapping:!0,tabSize:2,viewportMargin:1/0})}function Y(){n.runButton.addEventListener("click",()=>{X()}),n.stopButton.addEventListener("click",Z),n.resetButton.addEventListener("click",oe),n.exampleSelect.addEventListener("change",()=>ie(n.exampleSelect.value)),n.shareButton.addEventListener("click",()=>{S(),navigator.clipboard?.writeText(window.location.href),m("Shareable URL copied when clipboard access is available.")});for(let e of[n.backgroundMode,n.dataMode])e.addEventListener("change",()=>{k(),g()});for(let e of Object.values(r))e.on("change",g);for(let e of[n.backgroundUrl,n.dataUrl])e.addEventListener("input",g);n.statefulMaterialization.addEventListener("change",g)}async function X(){if(h)return;let e={controller:new AbortController,startedAt:performance.now()};h=e;try{E(!0),n.runtimeStats.textContent="",n.runtimeStats.hidden=!0,A(),ee(e),r.outputText.setValue(""),V(e,"Preparing inference\u2026");let t=await J("background",e.controller.signal);re(e.controller.signal);let s=i("data"),a=s==="text"?r.dataText.getValue():void 0,o=s==="url"?n.dataUrl.value.trim():void 0;if(s==="url"&&!o)throw new Error("Enter a data URL or switch to text input.");await $(e,{apiScriptUrl:new URL("browser/rdfjs-inference-engine.min.js",window.location.href).href,bundledRules:D,bundledRuleCount:R.length,backgroundSource:t,dataMode:s,statefulMaterialization:n.statefulMaterialization.checked,statefulStoreName:n.statefulMaterialization.checked?K(t,s==="url"?o??"":a??""):void 0,dataSource:a,dataUrl:o})}catch(t){if(ae(t))F(),e.statusMessage="Stopped. No more messages or quads will be processed.";else{let s=t instanceof Error?t.message:String(t);A(),e.statusMessage=`Error: ${s}`,r.outputText.setValue(s)}M(e)}finally{M(e),h===e&&(h=null),e.worker?.terminate(),E(!1)}}function $(e,t){return new Promise((s,a)=>{let o=G();e.worker=o;let f=()=>{o.terminate(),a(new DOMException("Inference was stopped.","AbortError"))};e.controller.signal.addEventListener("abort",f,{once:!0}),o.onmessage=I=>{let l=I.data;l.type==="status"?V(e,l.message):l.type==="runtime"?(e.runtimeMessage=l.message,C(e)):l.type==="append"?te(l.chunk):l.type==="result"?(e.statusMessage=l.status,l.output!==void 0?(A(),r.outputText.setValue(l.output)):F(),M(e),s()):l.type==="error"&&a(new Error(l.message))},o.onerror=I=>{a(new Error(I.message))},o.postMessage(t)})}function K(e,t){let s=[window.location.origin,window.location.pathname,e,t].join("\0");return`rdfjs-inference-engine:playground:${Q(s)}`}function Q(e){let t=3735928559,s=1103547991;for(let a=0;a<e.length;a+=1){let o=e.charCodeAt(a);t=Math.imul(t^o,2654435761),s=Math.imul(s^o,1597334677)}return t=Math.imul(t^t>>>16,2246822507)^Math.imul(s^s>>>13,3266489909),s=Math.imul(s^s>>>16,2246822507)^Math.imul(t^t>>>13,3266489909),`${(s>>>0).toString(36)}${(t>>>0).toString(36)}`}function G(){let e=`
...
`,t=new Blob([e],{type:"text/javascript"});return new Worker(URL.createObjectURL(t))}async function J(e,t){if(i(e)==="text")return e==="background"?r.backgroundText.getValue():r.dataText.getValue();let o=(e==="background"?n.backgroundUrl:n.dataUrl).value.trim();if(!o)throw new Error(`Enter a ${e==="background"?"background RDF":"data"} URL or switch to text input.`);m(`Fetching ${e==="background"?"background RDF":"input data"} before processing\u2026`);let f=await fetch(o,{signal:t});if(!f.ok)throw new Error(`Could not fetch ${o}: ${f.status} ${f.statusText}`);return f.text()}function Z(){h&&(m("Stopping after the current worker step\u2026"),n.stopButton.disabled=!0,h.controller.abort())}function E(e){n.runButton.disabled=e,n.stopButton.hidden=!e,n.stopButton.disabled=!e}function V(e,t){e.statusMessage=t,z(e)}function ee(e){C(e),e.elapsedTimer=window.setInterval(()=>C(e),250)}function M(e){e.finishedAt===void 0&&(e.finishedAt=performance.now()),e.elapsedTimer!==void 0&&(window.clearInterval(e.elapsedTimer),e.elapsedTimer=void 0),C(e)}function C(e){n.runtimeStats.textContent="",n.runtimeStats.hidden=!0,z(e)}function z(e){if(!e.statusMessage)return;let t=e.finishedAt===void 0?"Elapsed":"Total elapsed",s=[W(e.statusMessage),`${t} ${se(ne(e))}`];e.runtimeMessage&&s.push(W(e.runtimeMessage)),m(s.filter(Boolean).join(" \xB7 "))}function W(e){return e.trim().replace(/[.。]\s*$/,"")}function te(e){y+=e,u||(u=window.setTimeout(F,100))}function F(){if(u&&(window.clearTimeout(u),u=0),!y)return;let e=y;if(y="",r.outputText.replaceRange&&r.outputText.lastLine&&r.outputText.getLine){let t=r.outputText.lastLine();r.outputText.replaceRange(e,{line:t,ch:r.outputText.getLine(t).length})}else r.outputText.setValue(r.outputText.getValue()+e)}function A(){u&&(window.clearTimeout(u),u=0),y=""}function ne(e){return(e.finishedAt??performance.now())-e.startedAt}function se(e){let t=Math.max(0,e/1e3);if(t<10)return`${t.toFixed(1)} s`;if(t<60)return`${t.toFixed(0)} s`;let s=Math.floor(t/60),a=Math.floor(t%60).toString().padStart(2,"0");return`${s} min ${a} s`}function re(e){if(e.aborted)throw new DOMException("Inference was stopped.","AbortError")}function ae(e){return e instanceof DOMException&&e.name==="AbortError"}function k(){b(n.backgroundUrlPanel,i("background")==="url"),b(n.backgroundTextPanel,i("background")==="text"),b(n.dataUrlPanel,i("data")==="url"),b(n.dataTextPanel,i("data")==="text"),window.setTimeout(()=>{r.backgroundText.refresh(),r.dataText.refresh(),r.outputText.refresh()},0)}function b(e,t){e.hidden=!t}function i(e){return(e==="background"?n.backgroundMode.value:n.dataMode.value)==="url"?"url":"text"}function oe(){let e=w();c=!0,n.exampleSelect.value=e.id,n.backgroundMode.value=p.backgroundMode,n.dataMode.value=p.dataMode,n.statefulMaterialization.checked=!1,n.backgroundUrl.value="",n.dataUrl.value="",r.backgroundText.setValue(e.background),r.dataText.setValue(e.data),r.outputText.setValue(""),c=!1,k(),S(),m(`Reset to ${e.label}.`)}function le(){n.exampleSelect.textContent="";for(let e of x){let t=document.createElement("option");t.value=e.id,t.textContent=`${e.label} \u2014 ${e.backgroundFile} + ${e.dataFile}`,n.exampleSelect.appendChild(t)}n.exampleSelect.value=w().id}function ie(e){let t=T(e)??w();c=!0,B(t),c=!1,k(),S(),m(`Loaded ${t.label} from ${t.backgroundFile} and ${t.dataFile}.`)}function B(e){n.exampleSelect.value=e.id,n.backgroundMode.value="text",n.dataMode.value="text",n.statefulMaterialization.checked=U(e.id),n.backgroundUrl.value="",n.dataUrl.value="",r.backgroundText.setValue(e.background),r.dataText.setValue(e.data),r.outputText.setValue("")}function U(e){return e==="stateful-materialization"}function w(){return T("owl-skos-catalog")??x[0]}function T(e){return x.find(t=>t.id===e)}function pe(){let e=T(n.exampleSelect.value);if(e&&de(e))return e.id===w().id?{}:{example:e.id};let t={backgroundMode:i("background")===p.backgroundMode?void 0:i("background"),dataMode:i("data")===p.dataMode?void 0:i("data"),statefulMaterialization:n.statefulMaterialization.checked||void 0,backgroundUrl:n.backgroundUrl.value.trim()||void 0,dataUrl:n.dataUrl.value.trim()||void 0},s=r.backgroundText.getValue(),a=r.dataText.getValue();return s!==p.backgroundText&&(t.backgroundText=s),a!==p.dataText&&(t.dataText=a),t}function de(e){return i("background")==="text"&&i("data")==="text"&&n.statefulMaterialization.checked===U(e.id)&&n.backgroundUrl.value.trim()===""&&n.dataUrl.value.trim()===""&&r.backgroundText.getValue()===e.background&&r.dataText.getValue()===e.data}function ce(){let e=me(window.location.hash);if(e){let s=T(e);if(s){c=!0,B(s),c=!1;return}}let t=fe(window.location.hash);t&&(c=!0,n.backgroundMode.value=t.backgroundMode??p.backgroundMode,n.dataMode.value=t.dataMode??p.dataMode,n.statefulMaterialization.checked=t.statefulMaterialization??!1,n.backgroundUrl.value=t.backgroundUrl??"",n.dataUrl.value=t.dataUrl??"",t.backgroundText!==void 0&&r.backgroundText.setValue(t.backgroundText),t.dataText!==void 0&&r.dataText.setValue(t.dataText),c=!1)}function g(){c||(window.clearTimeout(q),q=window.setTimeout(S,400))}function S(){let e=ue(pe()),t=`${window.location.pathname}${window.location.search}${e?`#${e}`:""}`;window.history.replaceState(null,"",t)}function ue(e){if(e.example&&Object.keys(e).length===1)return`example=${encodeURIComponent(e.example)}`;let t=JSON.stringify(e);return t==="{}"?"":`state=${btoa(unescape(encodeURIComponent(t))).replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/g,"")}`}function me(e){if(!e.startsWith("#example="))return"";try{return decodeURIComponent(e.slice(9))}catch{return""}}function fe(e){let t=e.startsWith("#state=")?e.slice(7):"";if(!t)return null;try{let s=t.replace(/-/g,"+").replace(/_/g,"/").padEnd(Math.ceil(t.length/4)*4,"=");return JSON.parse(decodeURIComponent(escape(atob(s))))}catch{return null}}function m(e){n.status.textContent=e}function O(e){return d(e)}function L(e){return d(e)}function v(e){return d(e)}function d(e){let t=document.getElementById(e);if(!t)throw new Error(`Missing element #${e}`);return t}function he(e){return document.getElementById(e)}});we();})();
