...
const EX = 'https://example.org/shacl#';
...
async function main(): Promise<void> {
  const shapes = parseToQuads(readFileSync('examples/shacl-validation/shapes.n3', 'utf8'));
  const data = parseToQuads(readFileSync('examples/shacl-validation/input.trig', 'utf8'));

  const reasoner = new InferenceEngine();
  reasoner.load(shapes, {
    deterministicSkolem: true,
    skolemKey: 'examples/shacl-validation',
  });
...
function quadKey(quad: Quad): string {
  return [quad.subject, quad.predicate, quad.object, quad.graph].map((term) => termKey(term as Term)).join(' ');
}
