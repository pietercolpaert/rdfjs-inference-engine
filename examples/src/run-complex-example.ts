...
async function main(): Promise<void> {
  ...
  const ontology = parseToQuads(readFileSync('examples/complex/ontology.n3', 'utf8'));
  const reasoner = new InferenceEngine();

  reasoner.load(profile, ontology);
  mkdirSync(dirname('generated/complex-runtime.n3'), { recursive: true });
  reasoner.saveRuntime('generated/complex-runtime.n3');

  const data = parseToQuads(readFileSync('examples/complex/input.trig', 'utf8'));
  const closure = [...data, ...reasoner.infer(data)];
  const selected = selectedExpectedQuads();
  assertContainsQuads(closure, selected);
...
function assertContainsQuads(actual: Quad[], expected: Quad[]): void {
  const actualKeys = new Set(actual.map(quadKey));
  const missing = expected.filter((quad) => !actualKeys.has(quadKey(quad)));

  if (missing.length > 0) {
    throw new Error(`Complex OWL 2 RL example missed ${missing.length} expected entailment(s):\n${missing.map(quadKey).join('\n')}`);
  }
}
